...
- **The hunt map** (`/karte`): every `identified` (known but not yet acquired)
  drink placed on an interactive map of Germany by the postal code in its
  origin, so a tester passing through can see what is available nearby. Each
  place has "open in maps" links that work in any browser (OpenStreetMap, Apple
  Maps, Google Maps — the first two keep the Spezi name on the pin, Google gets
  the exact coordinates; on a touch device a `geo:` "Karten-App" link is added
  in front). Each place (`/karte/ort/{plz}.gpx`) and the whole map
  ...
  position stays in the browser and is never sent anywhere). Every Spezi detail
  page links back to its spot on the map. Coordinates come from bundled GeoNames
  tables (`src/Website/Catalog/Geo/`, regenerated by the `tools/geo/build-*.php`
  scripts).
...
- **Public map feed** (`GET /api/map/spezis.geojson`): the same pins as `/karte`
  ...
  origin resolves to a coordinate, carrying only the database id and the name.
