...
function verifyTrackedPackage(string $root, string $sqlPath, string $manifestPath, array $manifest): void
{
    ...
    if (!is_string($expectedSqlHash) || !is_string($actualSqlHash) || !hash_equals($expectedSqlHash, $actualSqlHash) || !is_string($sql)) {
...
SQL;
    ...
    $planPath = $root . '/var/primary-refresh/current/refresh-plan.json';
    ...
    if (!is_string($planContents) || !is_string($workbookHash) || preg_match('/\A[a-f0-9]{64}\z/D', $workbookHash) !== 1) {
    ...
    $manifest = [
        ...
        'refresh_plan_sha256' => hash('sha256', $planContents),
    ...
    verifyTrackedPackage($root, $sqlPath, $manifestPath, $manifest);
}
